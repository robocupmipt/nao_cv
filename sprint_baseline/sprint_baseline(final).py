...
#img image rename
marker_color_min = (5,110, 245)#Min color of red box#(60,105)
marker_color_max = (20, 130, 255)#Max color of red box

pattern_size = (4,5)#Pattern size of chessboard
X_DIM=640
Y_DIM=480
...
def ObjectCondition(center_x, allowed_gamma=0, X_DIM=X_DIM):
    PREVIOUS_TURN = -1#Default rotation is right
...
def BackCondition(line, center):
    if line is None or line.y_min>center.y:
        return 1
    return 0
def ShouldTurn(image, allowed_gamma=0, X_checkpoint=X_DIM//2,Y_checkpoint=Y_DIM//2,
               log_lines= True, use_lines=False,mode='chessboard'):
    ...
    object_condition = ObjectCondition(center[0],allowed_gamma, X_DIM=image.shape[0])
